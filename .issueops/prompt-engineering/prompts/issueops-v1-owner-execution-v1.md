...
10. 구현 진입 전에 required skill `issueops`의 "환경별 자동 세션 인계"를 따른다.
   이미 인계받은 owner는 인계문과 status의 실제 결정 기록·범위·종료점을 대조하고 이어간다.
   Orca에서 실행 중이라는 이유로 자동 인계를 다시 적용하거나 새 세션을 띄우지 않는다.
   아직 인계하지 않은 준비 세션만 Orca ready이면 새 세션, absent/unready이면 현재 세션으로
   자동 결정한다. 실행 방식이나 진행 여부를 묻지 않는다. 보류·범위 제한 등 최신 지시가 우선한다.
   자동 결정과 phase/claim 성공은 승인 범위를 넓히지 않는다. 확인 뒤에는 같은 lifecycle ID로
