...
- [x] G6: 계약 golden
  ...
  EVIDENCE: ok  	issueops/cmd/issueops/contractgolden	2.086s | ok  	issueops/cmd/issueops/issueopsapp	13.668s
- [x] G7: PR 계약 필수 절 4개
  CHECK: go test ./internal/domain/artifacttemplate -run TestPullRequestContractRequiresFourSections -count=1
  EVIDENCE: ok  	issueops/internal/domain/artifacttemplate	3.052s
- [x] G8: 구현 이슈 계약 필수 절 5개
  CHECK: go test ./internal/domain/artifacttemplate -run TestImplementationIssueContractRequiresFiveSections -count=1
  EVIDENCE: ok  	issueops/internal/domain/artifacttemplate	2.099s
- [x] G9: 가독성 검사 규칙
  CHECK: go test ./internal/domain/artifactreadability -count=1
  EVIDENCE: ok  	issueops/internal/domain/artifactreadability	1.813s
- [x] G10: create-issue confirm이 critical과 템플릿 누락을 intent 기록 전에 거부
  CHECK: go test ./cmd/issueops/issueopscli/remotecmd -run TestRemoteCreateIssueRefusesCriticalReadability -count=1
  EVIDENCE: ok  	issueops/cmd/issueops/issueopscli/remotecmd	1.935s
- [x] G11: create-child·create-pr confirm이 critical을 거부하고 기본 템플릿을 쓴다
  CHECK: go test ./cmd/issueops/issueopscli/remotecmd -run TestRemoteCreateChildAndPRRefuseCriticalReadability -count=1
  EVIDENCE: ok  	issueops/cmd/issueops/issueopscli/remotecmd	2.168s
- [x] G12: sync-issue·sync-pr confirm이 critical을 거부하고 원격 본문 warning을 보고
  ...
  EVIDENCE: ok  	issueops/internal/adapter/issueops	1.469s | ok  	issueops/cmd/issueops/issueopscli/remotecmd	1.647s
- [x] G13: reflect-completion이 원고를 요구하고 SHA·로컬 경로·과다 길이를 거부
  ...
  EVIDENCE: ok  	issueops/cmd/issueops/issueopscli/remotecmd	0.388s | ok  	issueops/internal/adapter/issueops	0.663s
- [x] G14: 진행 결과 구간에 하네스 값이 없음
  CHECK: go test ./internal/adapter/provider/issuebody -run TestCompletionSectionIsHumanReadable -count=1
  EVIDENCE: ok  	issueops/internal/adapter/provider/issuebody	0.647s
- [x] G15: 정리 경로가 이슈 본문을 쓰지 않음
  CHECK: go test ./internal/adapter/issueops -run TestCleanupPathsKeepAuditOutOfIssueBody -count=1
  EVIDENCE: ok  	issueops/internal/adapter/issueops	0.452s
- [x] G16: 계획 검토 한 줄 요약, 중단 지적 가림, 짧은 중단 판정의 반영·regress
  ...
  EVIDENCE: ok  	issueops/internal/adapter/provider/issuebody	0.243s | ok  	issueops/internal/adapter/issueops	0.722s
- [x] G17: implement·ai-slop-clean 전이가 두 모드에서 추적 사본을 쓴다
  CHECK: go test ./internal/adapter/issueops -run TestPhaseTransitionWritesTrackedMaterials -count=1
  EVIDENCE: ok  	issueops/internal/adapter/issueops	1.228s
- [x] G18: #513 추적 사본이 있고 무시되지 않는다
...
- [x] G25: 진행 결과 반영은 머지 증거를 계속 요구
  ...
  EVIDENCE: cannot reflect completion without provider-verified merge evidence
- [x] G26: regress는 중단 판정 반영을 계속 요구
  ...
  EVIDENCE: reflect the devil's-advocate findings to the issue before regressing
- [x] G27: 봉인 디렉터리는 계속 무시된다
  CHECK: git check-ignore -q .issueops/issues/513/artifact/plan.md
  EVIDENCE: (no output)
- [x] G28: 사본이 없는 기존 사이클에 warning
  CHECK: go test ./internal/adapter/issueops -run TestTrackedMaterialsMissingWarning -count=1
  EVIDENCE: ok  	issueops/internal/adapter/issueops	0.605s
