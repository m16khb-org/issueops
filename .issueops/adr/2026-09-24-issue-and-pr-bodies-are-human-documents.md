...
- Decision:
  ...
  3. The completion region is a human-written progress report: `reflect-completion --body-file`, required with `--confirm`, checked as a completion draft (full commit SHAs and local paths critical, 2,000-character cap). It still requires provider-verified merge evidence and stays in post-merge cleanup.
